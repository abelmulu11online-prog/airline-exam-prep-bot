...
public class PaymentNotificationDispatcher {
 ...
 record Work(long id,long request,String kind,long recipient,String language,int attempts) {}
 @Scheduled(fixedDelay=10000,initialDelay=10000)
 public void scheduled() {if(automatic) retryDue();}
 ...
 public void deliver(long id) {
  ...
  Work work=tx.execute(status->{
   ...
   var rows=store.jdbc().query("SELECT * FROM payment_notifications WHERE id=? AND status IN ('NEW','FAILED','SENDING') AND attempts<5 AND next_attempt_at<=?",
    (r,n)->new Work(r.getLong("id"),r.getLong("request_id"),r.getString("kind"),r.getLong("recipient_id"),r.getString("language"),r.getInt("attempts")+1),id,java.sql.Timestamp.from(clock.instant()));
   if(rows.isEmpty()) return null;
   store.jdbc().update("UPDATE payment_notifications SET status='SENDING',attempts=attempts+1,next_attempt_at=? WHERE id=?",
    java.sql.Timestamp.from(clock.instant().plusSeconds(120)),id);return rows.getFirst();
  });
  if(work==null) return;
  boolean sent=false;
  try {
  ...
  tx.executeWithoutResult(status->{
   settings.lock();
   store.jdbc().update("UPDATE payment_notifications SET status=?,sent_at=?,next_attempt_at=? WHERE id=?",
    delivered?"SENT":"FAILED",delivered?java.sql.Timestamp.from(clock.instant()):null,
    java.sql.Timestamp.from(clock.instant().plusSeconds(60L*work.attempts())),id);
   audit.append("SYSTEM","notification",(work.kind().equals("ADMIN_PENDING")?"ADMIN":"USER")+"_NOTIFICATION_"+(delivered?"SENT":"FAILED"),"PAYMENT",work.request(),"attempt="+work.attempts());
  });
 }
 public void retry(long request,String admin) {
  tx.executeWithoutResult(status->{
   settings.lock();payments.get(request);
   int changed=store.jdbc().update("UPDATE payment_notifications SET status='NEW',attempts=0,next_attempt_at=? WHERE request_id=? AND status='FAILED'",
